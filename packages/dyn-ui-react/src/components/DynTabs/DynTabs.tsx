...
export const DynTabs = forwardRef<DynTabsRef, DynTabsProps>(
  (
    ...
    const renderTab = useCallback((item: DynTabItem) => {
      ...
      // Data status following DynAvatar pattern
      const dataStatus = isActive ? 'active' : isDisabled ? 'disabled' : 'inactive';
    ...
    }, [currentActiveTab, size, variant, closable, internalId, dataTestId, setActiveTabId, handleKeyDown, handleTabClose]);
    ...
    }, [items, currentActiveTab, lazy, loadedTabs, internalId, animated, contentClassName, dataTestId, loadingComponent]);
